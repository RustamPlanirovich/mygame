  ...
  useEffect(() => {
    if (user) {
      loadGame().then(() => {
        ...
        /*
         * Очередь админских выдач (bigplan.md, пункт 9) — строго ПОСЛЕ загрузки: начисление
         * прибавляется к состоянию в памяти, и сделай мы это раньше, загруженный сейв его бы
         * затёр. Это же единственный путь для игрока, которому выдали, пока он был офлайн.
         */
    ...
    <div className="flex h-[100dvh] flex-col overflow-hidden bg-cyber-black text-cyber-text">
      ...
      */}
      <main className="relative min-h-0 flex-1 overflow-hidden">
        ...
        {/* Ориентир «что делать дальше» (пункты 20, 29): постоянный, а не одноразовое обучение */}
        <ScenarioGuide />
