...
`deposits`, `tickBuffers`, `tickSchedule`, `randomEventSchedule`, `adminGrant`) — там же лежат
их тесты. Новую логику тика добавлять туда, а не наращивать `gameStore`.

**Месторождения** (`src/core/systems/deposits.ts`, bigplan 38) — раскладка жилами и их
истощение. Единственное место, где живут веса редкости, размеры пятен, запасы клеток и таблица
«какому зданию нужна какая жила». Запас клетки задаётся в ЧАСАХ работы добытчика первого
уровня, а не в единицах: ставки у зданий разные. Выработанная жила = разрушенное здание;
отдельного флага в сейве нет, состояние вычисляется из остатка (`isTileRuined`).
